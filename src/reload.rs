...
/// Write `/opt/kap/bin/kap-post-start` script based on `[setup]` config.
...
pub fn write_post_start_script(cfg: &Config, shim_dir: &Path) -> anyhow::Result<()> {
    ...
    // If no setup config, remove stale script and return
    let any_enabled = setup.is_some_and(|s| s.claude_code || s.codex || s.gh || s.ssh_signing);
    if !any_enabled {
    ...
    let mut lines: Vec<&str> = vec![
        ...
        "set -e",
        "",
        "# Generated by kap from [setup] in kap.toml — do not edit.",
