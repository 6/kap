...
pub fn write_post_start_script(cfg: &Config, shim_dir: &Path) -> anyhow::Result<()> {
    ...
    if cfg.ssh_signing {
        lines.extend_from_slice(&[
            ...
            "# Read from the host gitconfig bind-mount (not ~/.gitconfig, which doesn't",
            "# exist — the devcontainer CLI skips copying it when GIT_CONFIG_GLOBAL is set).",
            "HOST_GITCONFIG=/opt/kap-host-gitconfig",
            "KEY=$(git config -f \"$HOST_GITCONFIG\" user.signingkey 2>/dev/null || true)",
            "EMAIL=$(git config -f \"$HOST_GITCONFIG\" user.email 2>/dev/null || true)",
            "if [ -n \"$KEY\" ]; then",
        ...
        lines.extend_from_slice(&[
            ...
            "SIGN_PROG=$(git config -f /opt/kap-host-gitconfig gpg.ssh.program 2>/dev/null || true)",
