...
pub fn write_post_start_script(cfg: &Config, shim_dir: &Path) -> anyhow::Result<()> {
    ...
    if cfg.ssh_signing {
        lines.extend_from_slice(&[
            ...
            "# Use `unset` (not empty string) — git 2.51+ treats GIT_CONFIG_GLOBAL='' as a file path.",
            "KEY=$(unset GIT_CONFIG_GLOBAL; git config --global user.signingkey 2>/dev/null || true)",
            "EMAIL=$(unset GIT_CONFIG_GLOBAL; git config --global user.email 2>/dev/null || true)",
            "if [ -n \"$KEY\" ]; then",
        ...
        lines.extend_from_slice(&[
            ...
            "SIGN_PROG=$(unset GIT_CONFIG_GLOBAL; git config --global gpg.ssh.program 2>/dev/null || true)",
