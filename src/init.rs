...
pub fn generate_overlay(
    ...
    // App service volumes: kap-bin (for shims + binary) + SSH agent socket
    let app_volumes = {
    ...
    // Docker Compose config to ensure /opt/kap/bin is on PATH for interactive shells.
    // zsh's ~/.zshrc (e.g. mise activate) rebuilds PATH, pushing remoteEnv additions to the end.
    // This config is mounted at /etc/zsh/zshenv which is sourced first in all zsh sessions.
    let path_config = if has_cli_tools {
        r#"configs:
  kap-path:
    content: |
...
"#
    ...
    let path_config_mount = if has_cli_tools {
...
{path_config}services:
  ...
  {service_name}:
    ...
    networks:
      kap_sandbox:
        ipv4_address: {app_ip}{app_volumes}{path_config_mount}
    depends_on:
      kap:
...
fn run_existing(project: &Path, devcontainer_dir: &Path, yes: bool, force: bool) -> Result<()> {
    ...
    let has_cli_tools = !detected.is_empty();
...
mod tests {
    ...
    #[test]
    fn overlay_mounts_kap_bin_when_tools_configured() {
        let compose = ComposeConfig::default();
        let overlay = generate_overlay("app", &compose, true, "172.28.0", "test", None, false);
        // kap-bin volume mounted for shims + binary
        assert!(overlay.contains("kap-bin:/opt/kap:ro"));
        // No per-tool shim configs (shims are on the shared volume now)
        assert!(!overlay.contains("cli-shim"));
        // PATH config mounted at /etc/zsh/zshenv for interactive shells
        assert!(overlay.contains("kap-path"));
        assert!(overlay.contains("/etc/zsh/zshenv"));
    }
    ...
    fn overlay_includes_global_config_mount() {
        let compose = ComposeConfig::default();
        let overlay = generate_overlay("app", &compose, false, "172.28.0", "test", None, true);
