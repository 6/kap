...
pub fn generate_overlay(
    ...
    format!(
        r#"# Generated by kap — DO NOT EDIT. Regenerated on each `kap init-env` run.
...
fn run_existing(project: &Path, devcontainer_dir: &Path, yes: bool) -> Result<()> {
    ...
    if dc_obj.get("initializeCommand").is_some() {
        notes.push(
            "initializeCommand already set. Add `kap init-env` to your existing command."
                .to_string(),
        );
    } else {
        dc_obj["initializeCommand"] = serde_json::Value::String("kap init-env".to_string());
    }
...
fn run_new(project: &Path, devcontainer_dir: &Path) -> Result<()> {
    ...
    println!("  kap.toml            - guard config (edit allowed domains here)");
...
fn generate_devcontainer_json(project_name: &str) -> String {
  ...
  "initializeCommand": "kap init-env",
  "remoteUser": "vscode"
}}
"#
    )
...
mod tests {
    ...
    fn existing_project_creates_overlay_and_updates_json() {
        ...
        assert_eq!(compose_arr[1], OVERLAY_FILENAME);
        assert_eq!(updated["initializeCommand"], "kap init-env");
