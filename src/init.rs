...
pub fn generate_overlay(service_name: &str, compose: &ComposeConfig, has_gh: bool) -> String {
    let image_yaml = compose.image_yaml("    ");
    let gh_volume = if has_gh {
        "\n    volumes:\n      - ./gh-shim.sh:/usr/local/bin/gh:ro"
    } else {
        ""
    };
    format!(
...
services:
  ...
  {service_name}:
    ...
    networks:
      devg_sandbox:
        ipv4_address: 172.28.0.2{gh_volume}
    depends_on:
      devg:
...
fn run_existing(project: &Path, devcontainer_dir: &Path) -> Result<()> {
    ...
    write_file(
        &overlay_path,
        &generate_overlay(&service_name, &compose_config, false),
    )?;
...
fn run_new(project: &Path, devcontainer_dir: &Path) -> Result<()> {
    ...
    write_file(
        ...
        &generate_overlay("app", &compose_config, false),
    )?;
...
mod tests {
    ...
    fn overlay_with_build_config() {
        ...
        let overlay = generate_overlay("app", &compose, false);
