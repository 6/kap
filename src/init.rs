...
pub fn generate_overlay(
    ...
    let has_cli_tools = cli_tool_names.is_some_and(|t| !t.is_empty());
    ...
    // CLI shim config: mount the shim script at /usr/local/bin/<tool> for each
    // configured tool. This shadows any real binary at that path (e.g. gh installed
...
fn run_existing(project: &Path, devcontainer_dir: &Path, yes: bool, force: bool) -> Result<()> {
    ...
    let detected_names: Vec<String> = detected.iter().map(|t| t.name.to_string()).collect();
    ...
    let cli_names = if detected_names.is_empty() {
        None
    } else {
        Some(detected_names.as_slice())
    };
    write_file(
        &overlay_path,
        &generate_overlay(
...
mod tests {
    ...
    fn overlay_mounts_kap_bin_and_shims_when_tools_configured() {
        let compose = ComposeConfig::default();
        let tools: Vec<String> = vec!["gh".into(), "aws".into()];
        let overlay = generate_overlay(
            "app",
            &compose,
            Some(&tools),
        ...
        // kap-bin volume mounted for kap binary
        assert!(overlay.contains("kap-bin:/opt/kap:ro"));
        // Shim config mounted at /usr/local/bin/<tool> for each tool
    ...
    fn overlay_contains_hostname() {
        let compose = ComposeConfig::default();
        let overlay =
            generate_overlay("app", &compose, None, "172.28.0", "my-project", None, false);
    ...
    fn overlay_omits_ssh_agent_when_unset() {
        let compose = ComposeConfig::default();
        let overlay = generate_overlay("app", &compose, None, "172.28.0", "test", None, false);
    ...
    fn overlay_includes_ssh_agent_and_cli_volumes() {
        let compose = ComposeConfig::default();
        let tools: Vec<String> = vec!["gh".into()];
        let overlay = generate_overlay(
            "app",
            &compose,
            Some(&tools),
    ...
    fn overlay_includes_global_config_mount() {
        let compose = ComposeConfig::default();
        let overlay = generate_overlay("app", &compose, None, "172.28.0", "test", None, true);
