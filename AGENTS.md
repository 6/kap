...
`docker-compose.kap.yml` is a **generated file** (gitignored). It is regenerated by `kap init-env` on every `kap up`, so it always matches the installed kap version.
