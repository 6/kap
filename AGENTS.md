...
5. **Remote access daemon** (:19420): runs on the host (not in Docker). HTTP server with token-based auth for monitoring and steering devcontainers from a phone. QR code pairing, WebSocket streaming, web UI served from the binary. Start with `devg remote start`.
...
**Do NOT use `docker compose up` directly** to manage devcontainers. Always use `devg up` (which wraps `devcontainer up`). The devcontainer CLI adds labels that `devcontainer exec` needs to find containers. Manual `docker compose up` strips these labels and breaks `devg exec`.
...
devg up --reset                             # full recreate
devg exec devg status                       # verify all checks pass
devg exec .devcontainer/smoke-test.sh       # run smoke tests
```

`--reset` is required whenever `.env`, the overlay template, or shim scripts change. Without it, `devg up` reuses the existing container with stale env vars and volume mounts.

## Compose overlay

`docker-compose.devg.yml` is a **generated file** (gitignored). It is regenerated by `devg init-env` on every `devg up`, so it always matches the installed devg version.
