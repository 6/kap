...
- **gh CLI proxy** - `gh` commands are proxied through the sidecar with a command allowlist
...
The `gh` CLI proxy lets the app container run `gh` commands without having GH_TOKEN. Commands are forwarded to the sidecar which executes them with credentials.
...
`gh auth token` and `gh api` are always blocked regardless of the allow list.
