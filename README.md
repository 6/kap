...
```bash
devg remote start    # starts HTTP daemon on :19420, shows QR code
```
...
```bash
devg remote pair     # show QR code again
devg remote devices  # list paired devices
devg remote revoke <id>  # revoke a device
```
...
| `devg remote start` | Start the remote access daemon |
| `devg remote pair` | Show QR code for phone pairing |
| `devg remote devices` | List paired devices |
| `devg remote revoke <id>` | Revoke a paired device |
