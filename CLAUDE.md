...
Single Rust binary with two proxy layers:
...
2. **MCP proxy** (:3129) — Reverse proxy for remote Streamable HTTP MCP servers. Provides tool-level filtering and credential isolation. Only starts when `[mcp]` is in config.
